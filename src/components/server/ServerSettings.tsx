...
import {
  ...
  countPendingJoinRequests, leaveServer,
            ...
            <div className="flex items-center gap-2">
              ...
              {(() => {
                ...
                const style = p === 'free'
                  ...
                  : p === 'pro'
                  ? { background: 'rgba(var(--theme-accent-rgb), 0.12)', color: 'var(--theme-accent)', border: '1px solid rgba(var(--theme-accent-rgb), 0.28)' }
                  : { background: 'rgba(217,70,239,0.10)', color: 'rgb(232,121,249)', border: '1px solid rgba(217,70,239,0.30)' };
                return (
                  <span className="text-[8px] font-bold uppercase tracking-[0.12em] px-1.5 py-0.5 rounded-md shrink-0 leading-none"
                    style={style}>{p}</span>
                );
              })()}
            </div>
        ...
        <div className="flex gap-1 px-8 py-2.5 border-b border-[rgba(var(--glass-tint),0.04)] overflow-x-auto custom-scrollbar">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[12px] font-semibold transition-all shrink-0 ${tab === t.id ? 'bg-[var(--theme-accent)]/10 text-[var(--theme-accent)]' : 'text-[var(--theme-secondary-text)]/40 hover:text-[var(--theme-text)] hover:bg-[rgba(var(--glass-tint),0.04)]'}`}>
              {t.icon} {t.label}
              {!!t.badge && t.badge > 0 && (
                <span className="ml-0.5 min-w-[16px] h-[16px] px-1 rounded-full text-[9px] font-bold flex items-center justify-center bg-[var(--theme-accent)] text-black shadow-[0_0_6px_rgba(var(--theme-accent-rgb),0.45)]">
...
function GeneralTab({ server, canEdit, isOwner, onSave, onDelete, onLeave, showToast }: {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Planlar — state single source of truth: server.plan ── */}
      {(() => {
        type PlanKey = 'free' | 'pro' | 'ultra';
        const currentPlan: PlanKey = server.plan === 'pro' || server.plan === 'ultra' ? server.plan as PlanKey : 'free';
        const RANK: Record<PlanKey, number> = { free: 0, pro: 1, ultra: 2 };
        const cardState = (target: PlanKey): 'current' | 'passed' | 'upgrade' => {
          if (RANK[target] === RANK[currentPlan]) return 'current';
          return RANK[target] < RANK[currentPlan] ? 'passed' : 'upgrade';
        };
        // Şu an yalnızca Pro aktif; Ultra "yakında". Upgrade kontrollü yapılır.
        const ULTRA_AVAILABLE = false;
        const upgradeLabel: Record<PlanKey, string> = { free: 'Free', pro: "Pro'ya Yükselt", ultra: "Ultra'ya Yükselt" };

        const cards: Array<{ id: PlanKey; name: string; sub: string; features: string[]; popular?: boolean }> = [
          { id: 'free',  name: 'Free',  sub: 'Varsayılan plan',          features: ['4 sistem odası (20 kişi)', '2 özel oda (10 kişi)', '100 kullanıcı kapasitesi', 'Standart ses kalitesi'] },
          { id: 'pro',   name: 'Pro',   sub: '+140 kullanıcı kapasitesi', features: ['4 sistem odası (40 kişi)', '4 özel oda (20 kişi)', '240 kullanıcı kapasitesi', 'Yüksek ses kalitesi', 'Daha düşük gecikme', 'Pro rozeti'], popular: true },
          { id: 'ultra', name: 'Ultra', sub: 'Maksimum performans',      features: ["Pro'nun tüm özellikleri ×2", 'En düşük gecikme', 'Öncelikli performans', 'Premium görünüm'] },
        ];
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <div className="text-[13px] font-bold text-[var(--theme-text)]">
                  {currentPlan === 'ultra' ? 'Ultra plandasın' : 'Sunucuyu Yükselt'}
                </div>
                <div className="text-[10px] text-[var(--theme-text)] opacity-40">
                  {currentPlan === 'ultra'
                    ? 'Maksimum tier — yükseltilecek üst plan yok'
                    : 'Sunucunun gücünü artır, daha fazla kullanıcı ve oda kapasitesine ulaş'}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {cards.map(c => {
                ...
                const isPopular = !!c.popular && !isCurrent && !isPassed;

                // Kart stil seçimi — mevcut plan öne çıkar, passed sönük, upgrade vurgulu.
                const cardStyle: React.CSSProperties = isCurrent
                  ? { background: 'linear-gradient(160deg, rgba(var(--theme-accent-rgb), 0.10), rgba(var(--theme-accent-rgb), 0.04))', border: '1.5px solid rgba(var(--theme-accent-rgb), 0.35)', boxShadow: '0 0 28px rgba(var(--theme-accent-rgb), 0.10)' }
                  : isPassed
                  ? { background: 'rgba(var(--glass-tint), 0.02)', border: '1px dashed rgba(var(--glass-tint), 0.07)' }
                  : c.id === 'pro'
                  ? { background: 'linear-gradient(160deg, rgba(var(--theme-accent-rgb), 0.08), rgba(var(--theme-accent-rgb), 0.03))', border: '1px solid rgba(var(--theme-accent-rgb), 0.15)' }
                  : { background: 'rgba(var(--glass-tint), 0.03)', border: '1px solid rgba(var(--glass-tint), 0.06)' };

                const upgradeDisabled = c.id === 'ultra' && !ULTRA_AVAILABLE;

                return (
                  <div key={c.id} className={`rounded-xl p-4 flex flex-col relative ${isPassed ? 'opacity-50' : ''}`} style={cardStyle}>
                    {isCurrent && (
                      <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-[8px] font-bold tracking-wider uppercase flex items-center gap-1"
                        style={{ background: 'var(--theme-accent)', color: 'var(--theme-text-on-accent, #000)' }}>
                        <Crown size={9} /> Mevcut Plan
                      </span>
                    )}
                    {isPopular && (
                    ...
                    <div className={`text-[12px] font-bold mb-1 ${isCurrent || c.id === 'pro' ? 'text-[var(--theme-accent)]' : 'text-[var(--theme-text)]'} ${isCurrent ? 'mt-1' : ''}`}>{c.name}</div>
                    <div className={`text-[9px] mb-3 opacity-50 ${isCurrent ? 'text-[var(--theme-accent)]' : 'text-[var(--theme-secondary-text)]'}`}>{c.sub}</div>
                    <div className="space-y-2 flex-1">
                      {c.features.map(f => <div key={f}><PlanFeature text={f} accent={c.id !== 'free' && !isPassed} /></div>)}
                    </div>

                    {isCurrent && (
                      <button disabled className="w-full h-9 mt-4 rounded-lg text-[11px] font-bold cursor-default flex items-center justify-center gap-1.5"
                        style={{ background: 'rgba(var(--theme-accent-rgb), 0.15)', color: 'var(--theme-accent)', border: '1px solid rgba(var(--theme-accent-rgb), 0.3)' }}>
                        <Crown size={11} /> Mevcut Plan
                      </button>
                    )}
                    {isPassed && (
                      <button disabled className="w-full h-9 mt-4 rounded-lg text-[11px] font-semibold text-[var(--theme-secondary-text)]/40 cursor-not-allowed" style={{ background: 'rgba(var(--glass-tint), 0.03)', border: '1px solid rgba(var(--glass-tint), 0.05)' }}>
                        Geçildi
                      </button>
                    )}
                    {isUpgrade && !upgradeDisabled && (
                      <button onClick={() => showToast(`${c.name} plan yakında aktif olacak`)}
                        ...
                        <Zap size={12} /> {upgradeLabel[c.id]}
                      </button>
                    )}
                    {isUpgrade && upgradeDisabled && (
                      <button disabled title="Yakında aktif olacak" className="w-full h-9 mt-4 rounded-lg text-[11px] font-semibold text-[var(--theme-secondary-text)]/30 cursor-not-allowed" style={{ background: 'rgba(var(--glass-tint), 0.04)', border: '1px solid rgba(var(--glass-tint), 0.06)' }}>
                        Yakında
                      </button>
                    )}
                  </div>
      ...
      {/* Sunucuyu Sil (owner) / Sunucudan Ayrıl (üye) */}
      {isOwner ? (
        ...
        <div className="pt-3 border-t border-[rgba(var(--glass-tint),0.05)]">
          <div className="p-4 rounded-xl flex items-center justify-between" style={{ background: 'rgba(var(--glass-tint), 0.03)', border: '1px solid rgba(var(--glass-tint), 0.06)' }}>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: 'rgba(239,68,68,0.08)' }}><X size={14} className="text-red-400/60" /></div>
              <div><div className="text-[12px] font-semibold text-[var(--theme-text)]">Sunucudan Ayrıl</div><div className="text-[10px] text-[var(--theme-secondary-text)]/40 mt-0.5">Üyelik ve rollerin silinir, tekrar katılmak için davet gerekir</div></div>
            </div>
            <button onClick={() => setLeaveModal(true)} className="h-9 px-5 rounded-lg text-[11px] font-bold bg-red-500/10 text-red-400 hover:bg-red-500/25 border border-red-500/15 transition-colors shrink-0">Ayrıl</button>
          </div>
        </div>
      )}
