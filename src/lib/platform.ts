...
/** Capacitor native ortamında mı çalışıyoruz? */
...
export const isElectron = (): boolean => {
  return typeof window !== 'undefined' && typeof window.electronApp !== 'undefined';
};
...
export const getAppVersion = async (): Promise<string> => {
  if (isElectron()) {
    try {
      return await window.electronApp!.getVersion();
    } catch {
      // fallthrough
    }
  }
...
export const setTrayChannel = (name: string | null): void => {
  if (isElectron()) {
    window.electronApp?.setTrayChannel?.(name);
  }
};
